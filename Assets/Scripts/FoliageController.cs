...
public class FoliageController : MonoBehaviour
{
    ...
    [SerializeField]
    [InfoBox("The range of time (in seconds) between spawns " +
        "once each piece of foliage has left the screen.")]
    private Vector2
        spawnDelay;
    ...
    private void Start()
    {
        ...
        StartCoroutine(FoliageSpawner());

    }

    private IEnumerator FoliageSpawner()
    {
        // random delay
    ...
    private IEnumerator FoliageMover()
    {
        ...
        //flip X half the time
        currentFoliage.GetComponent<SpriteRenderer>().flipX = (Random.Range(0, 2) == 0);
